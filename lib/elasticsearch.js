...
exports.search = async function search(query, page, sloppy) {
  ...
  let meta = await doRequest(
    "/aspen/file/_search",
    "POST",
    buildQueryObject(query, page, sloppy, true)
  )

  // Second pass: what we annotate and send to the client.
  let body = await doRequest(
    "/aspen/file/_search",
    "POST",
    buildQueryObject(query, page, sloppy, false)
  )
  ...
  if (body.hits && body.hits.hits && body.hits.hits.length) {
    for (let index = 0; index < body.hits.hits.length; index++) {
      ...
      let content =
        h["text.english"] && h["text.english"][0] != null
          ? h["text.english"][0]
          : h["text"] && h["text"][0] != null
          ? h["text"][0]
          : null
      if (!content) {
        console.error(
          `Can't get meta content for hit ${index} of query: ${query}`
        )
        continue
      }
      ...
      // o = cursor in original text document
      // c = cursor in content with __HLE__ and __HLS__ markers
